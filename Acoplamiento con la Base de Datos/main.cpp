...
void change_last(int i);   //funcion que se encarga de depurar el vector
void show_vector();        //enseña todos los procutos
void edit_Product();      //edita algún producto
void text_backup();      //
void read_backup();
...
void Add_Product(){
    ...
    int p=0;
    while( (b!=nuevo[p].get_code())&( nuevo[p].get_code()!="Sin Codigo"   )   ){
        p++;
    }
    if(  nuevo[p].get_code()==b   ){
        cout<<"Codigo DE Barras repetido, no se agrego producto"<<endl;
        return;
    }
...
void Find_Product(){
    ...
    cin>>c;
    while((nuevo[i].get_code()!=c)&(nuevo[i].get_code()!="Sin Codigo") ){
        i++;
    }
    if(nuevo[i].get_code()=="Sin Codigo"){
        cout<<"Producto No encontrado Por Codigo"<<endl;
        return;
    }
...
void change_last(int i){//funcion que solamente intercambia los datos del
                     //vector que se desea eliminar y el ultimo para que
                     //no haya huecos en el vector
...
void text_backup(){
    FILE *ptr=NULL;
    if(((ptr=fopen("backup.txt","r"))==NULL) ){
        cout<<"No se pudo abrir el Archivo o no hay base de tados aun"<<endl;
        ///ponemos el codigo aquí cuando no hay base de dato
        menu();
    }
    else{
        fclose(ptr);
        ///primero leeremos lo que tiene el archivo de texto
        read_backup();
    }
    return;
}
...
void read_backup(){
    ...
    while(!feof(ptr)){
        ...
        switch(i){
        case 1:
            textf.erase(textf.size()-1,1);
            nuevo[j].set_Name(textf);
            break;
        case 2:
            textf.erase(textf.size()-1,1);
            nuevo[j].set_Bar_Code(textf);
            break;
        case 3:
            nuevo[j].set_Price(atof(text));
            break;
        case 4:
            nuevo[j].set_Limit(  atof(text)  );
            break;
        case 5:
            nuevo[j].set_Exist( atof(text));
            break;
        }
        i++;
        if(i==6){
